...
## 2026-08-06 — Day 4 feature additions (UI / audio / accessibility)
- Added a **pause menu**: press `P` or `Esc` during play to freeze the round
  (`Time.timeScale = 0`) and show a PAUSED overlay with **Resume / Restart /
  Quit Game**. `GameManager` gained `IsPaused`, `Pause()`, `Resume()`,
  `TogglePause()`; the timer, player and enemy are all frozen while paused.
- Added on-page **controls / how-to-play** text to the start page, plus two
  clickable accessibility buttons (**Mute (M)** and **High Contrast (C)**) so
  settings are reachable without the keyboard. `SettingsManager` now exposes
  `Instance`, `ToggleMute()` and `ToggleContrast()` for the menu buttons.
- Updated the in-game help overlay (`I`) to list the pause keys and the new
  no-entry exit behaviour.
